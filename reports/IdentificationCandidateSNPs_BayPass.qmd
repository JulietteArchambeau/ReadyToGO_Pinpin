...
```{r ExtractSummaryOutputs, eval=T}
...
We can check the convergence across the **5 independent runs** (different `-seed` options) for each climatic variable and for the empirical p-values (`eBP`), the Bayes Factor (`BF`) and the regression coefficients (`beta`).
...
## Identify and export candidate SNPs

Number of candidate SNPs identified using different thresholds ($BF > 5$, $BF > 10$,$BF > 20$ or $eBP > 3$) for each climatic variable:
...
We use $BF > 10$ to identify the candidate SNPs:
...
1 SNP is associated with three climatic variables.

We export the list of candidate SNPs (`r unique(cand$snp) %>%  length()` SNPs).
