...
```{r CalculatePairwiseFstMatrix, eval=F}
fst_mat <- geno %>% 
...
```{r LoadPairwiseFstMatrix, echo=F}
fst_mat <- readRDS(file=here("outputs/GDM/fst_matrix.rds"))
```

### Scaling and formatting the $F_{ST}$ matrix

The pairwise $F_{ST}$ matrix contains `r length(fst_mat[which(fst_mat <0)])` **negative values**. It generally means there is more variation within than among populations and is likely to result from uneven sample sizes.
...
```{r PlotPredictedClimaticDistanceVsObservededGeneticDistance, fig.width=5, fig.height=5}
...
```{r PlotObservedVsPredictedGeneticDistance, fig.width=5, fig.height=5}
...
```{r MakePlotsIsplines, fig.width=14, fig.height=10, results="hide"}
...
ggsave(here("figs/GDM/GOMaps_PopLocations_Avg.pdf"), p, width=6,height=4, device="pdf")
