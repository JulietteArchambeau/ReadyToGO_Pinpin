...
tab <- tibble(ind = as.data.frame(geno@tab) %>% row.names(),
              pop = str_sub(ind,1,3)) %>% 
  arrange(pop)
...
We compare the list of the populations and the number of individuals in each population from the `genepop` file with the list of populations that Adé will use to calculate the genomic offset and the number of individuals in each population. Below, the column `nb_ind_genepop` corresponds to the number of individuals in each population from the `genepop` file and the column `nb_ind_Ade` corresponds to the number of individuals from each population in Adé's file.

```{r ComparingListPopulationsFromAdeFile}
ade_pop <- read_csv(here("data/genomic_data/POP_GLOBAL_GO_GOOD.csv"), show_col_types = FALSE) %>% 
  setNames(c("pop","nb_ind_Ade"))

left_join(n_pops, ade_pop, by="pop") %>% 
  mutate(nb_diff = nb_ind_genepop - nb_ind_Ade) %>% 
  kable_mydf()
```

We see that the number of individuals is not the same for four populations (FUE, LCO, TAL and MAU) and that the COM population is included in the `genepop` file but not in Adé's file.
